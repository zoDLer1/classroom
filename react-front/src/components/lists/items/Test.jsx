...
function Test({ id, template_info: { name, description }, passed_test, permissions }) {
    ...
    return (
        <div onClick={open} className={cx('transition-box_shadow duration-400 rounded-2xs', { 'my-4 shadow-md': condition })}>
            <div className={cx('group flex-ic-jb cursor-pointer p-3 rounded-2xs border-b border-solid border-black border-opacity-0 hover:bg-primary-100', { 'bg-primary-100 rounded-b-none !border-opacity-10': condition })}>
                <div className='flex-ic gap-5'>
                    ...
                    <p className='font-medium text-gray-750 text-base max-w-[400px] text-sizing select-none'>{name}</p>
                </div>
            ...
            <div className={cx('h-0 overflow-hidden flex-vertical px-5 transition-height duration-300', { 'h-60 pt-3': condition })} onClick={(evt) => evt.stopPropagation()}>
                <div className='flex-1 flex-vertical gap-5'>
                    <div className='text-end pr-10'>
                        <div className='flex-ic-je gap-3 text-sm text-opacity-50 text-black'>
