...
import { faCheck, faUsers, faSpinner, faPen, faTrash, faPalette } from '@fortawesome/free-solid-svg-icons'
import Input from 'components/UI/inputs/Input'
import { useOpen } from 'hooks/globalUIContent/useOpen'
import { useEffect } from 'react'
import { useMenu } from 'hooks/globalUIContent/useGlobalUI'
            ...
            <DefaultLink disabled={editMode} to={`/classes/${value.id}`} onClick={(evt) => evt.stopPropagation()} style={{ backgroundColor: `#${value.color_info.value}` }} className={css.header}>
                <div className={css.edit_layer} >
                    <div className={css.input}>
                        <Input onKeyUp={onKeyEnter} style={{ mixWidth: 200 }} styleType={'light'} readOnly={!editMode} form={{ isSubmitting: loading, errors: {}, touched: {} }} field={{ value: value.name, onChange: (evt) => setName(evt.target.value) }}  >
